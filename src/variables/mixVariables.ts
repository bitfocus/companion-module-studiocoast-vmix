...
export const mixValues = async (instance: VMixInstance): Promise<Map<string, CompanionVariableValue>> => {
  ...
  const setVariables = async (mix: Mix, selected: boolean = false) => {
    ...
    for (const type of mixTypes) {
      ...
      variables.set(`mix_${id}_${type}`, mix.program)
      variables.set(`mix_${id}_${type}_name`, await instance.data.getInputTitle(mix.program))
      variables.set(`mix_${id}_${type}_guid`, input.key)
      variables.set(`mix_${id}_${type}_playing`, (input.state === 'Running').toString())
      variables.set(`mix_${id}_${type}_loop`, input.loop.toString())
      variables.set(`mix_${id}_${type}_mute`, inputAudio.toString())
      variables.set(`mix_${id}_${type}_audio`, (!inputAudio).toString())
      variables.set(`mix_${id}_${type}_meterf1`, volumeTodB((input.meterF1 || 0) * 100).toFixed(1))
      variables.set(`mix_${id}_${type}_meterf2`, volumeTodB((input.meterF2 || 0) * 100).toFixed(1))
      ...
      if (audioLevel) {
        ...
        variables.set(`mix_${id}_${type}_meterf1_avg_1s`, volumeTodB(audioLevelData.s1MeterF1Avg * 100).toFixed(1))
        variables.set(`mix_${id}_${type}_meterf2_avg_1s`, volumeTodB(audioLevelData.s1MeterF2Avg * 100).toFixed(1))
        variables.set(`mix_${id}_${type}_meterf1_avg_3s`, volumeTodB(audioLevelData.s3MeterF1Avg * 100).toFixed(1))
        variables.set(`mix_${id}_${type}_meterf2_avg_3s`, volumeTodB(audioLevelData.s3MeterF2Avg * 100).toFixed(1))
        variables.set(`mix_${id}_${type}_meterf1_peak_1s`, volumeTodB(audioLevelData.s1MeterF1Peak * 100).toFixed(1))
        variables.set(`mix_${id}_${type}_meterf2_peak_1s`, volumeTodB(audioLevelData.s1MeterF2Peak * 100).toFixed(1))
        variables.set(`mix_${id}_${type}_meterf1_peak_3s`, volumeTodB(audioLevelData.s3MeterF1Peak * 100).toFixed(1))
        variables.set(`mix_${id}_${type}_meterf2_peak_3s`, volumeTodB(audioLevelData.s3MeterF2Peak * 100).toFixed(1))
      }

      const inputDuration = calcDuration(input)

      if (inputDuration !== null) {
        variables.set(`mix_${id}_${type}_duration`, inputDuration.ms)
      }

      const inputRemaining = calcRemaining(input)

      if (inputRemaining !== null) {
        variables.set(`mix_${id}_${type}_remaining`, inputRemaining.ms)
      }

      if (!(instance.config.strictInputVariableTypes && !instance.config.variablesShowInputPosition)) {
        variables.set(`mix_${id}_${type}_position_panx`, input.inputPosition?.panX ?? '')
        variables.set(`mix_${id}_${type}_position_pany`, input.inputPosition?.panY ?? '')
        variables.set(`mix_${id}_${type}_position_zoomx`, input.inputPosition?.zoomX ?? '')
        variables.set(`mix_${id}_${type}_position_zoomy`, input.inputPosition?.zoomY ?? '')
        variables.set(`mix_${id}_${type}_position_cropx1`, input.inputPosition?.cropX1 ?? '')
        variables.set(`mix_${id}_${type}_position_cropx2`, input.inputPosition?.cropX2 ?? '')
        variables.set(`mix_${id}_${type}_position_cropy1`, input.inputPosition?.cropY1 ?? '')
        variables.set(`mix_${id}_${type}_position_cropy2`, input.inputPosition?.cropY2 ?? '')
        variables.set(`mix_${id}_${type}_cc_hue`, input.cc?.hue ?? '')
        variables.set(`mix_${id}_${type}_cc_saturation`, input.cc?.saturation ?? '')
        variables.set(`mix_${id}_${type}_cc_liftr`, input.cc?.liftR ?? '')
        variables.set(`mix_${id}_${type}_cc_liftg`, input.cc?.liftG ?? '')
        variables.set(`mix_${id}_${type}_cc_liftb`, input.cc?.liftB ?? '')
        variables.set(`mix_${id}_${type}_cc_lifty`, input.cc?.liftY ?? '')
        variables.set(`mix_${id}_${type}_cc_gammar`, input.cc?.gammaR ?? '')
        variables.set(`mix_${id}_${type}_cc_gammag`, input.cc?.gammaG ?? '')
        variables.set(`mix_${id}_${type}_cc_gammab`, input.cc?.gammaB ?? '')
        variables.set(`mix_${id}_${type}_cc_gammay`, input.cc?.gammaY ?? '')
        variables.set(`mix_${id}_${type}_cc_gainr`, input.cc?.gainR ?? '')
        variables.set(`mix_${id}_${type}_cc_gaing`, input.cc?.gainG ?? '')
        variables.set(`mix_${id}_${type}_cc_gainb`, input.cc?.gainB ?? '')
        variables.set(`mix_${id}_${type}_cc_gainy`, input.cc?.gainY ?? '')
      }

      for (let i = 1; i < 11; i++) {
        variables.set(`mix_${id}_${type}_layer_${i}_number`, '')
        variables.set(`mix_${id}_${type}_layer_${i}_name`, '')
        variables.set(`mix_${id}_${type}_layer_${i}_key`, '')
      }

      for (const layer of input.overlay || []) {
        const inputLayer = await instance.data.getInput(layer.key)

        variables.set(`mix_${id}_${type}_layer_${layer.index + 1}_number`, inputLayer?.number || '')
        variables.set(`mix_${id}_${type}_layer_${layer.index + 1}_name`, inputLayer?.shortTitle || inputLayer?.title || '')
        variables.set(`mix_${id}_${type}_layer_${layer.index + 1}_key`, layer.key)

        if (!(instance.config.strictInputVariableTypes && !instance.config.variablesShowInputLayerPosition)) {
          variables.set(`mix_${id}_${type}_layer_${layer.index + 1}_panx`, layer.panX ?? '')
          variables.set(`mix_${id}_${type}_layer_${layer.index + 1}_pany`, layer.panY ?? '')
          variables.set(`mix_${id}_${type}_layer_${layer.index + 1}_x`, layer.x ?? '')
          variables.set(`mix_${id}_${type}_layer_${layer.index + 1}_y`, layer.y ?? '')
          variables.set(`mix_${id}_${type}_layer_${layer.index + 1}_zoomx`, layer.zoomX ?? '')
          variables.set(`mix_${id}_${type}_layer_${layer.index + 1}_zoomy`, layer.zoomY ?? '')
          variables.set(`mix_${id}_${type}_layer_${layer.index + 1}_width`, layer.width ?? '')
          variables.set(`mix_${id}_${type}_layer_${layer.index + 1}_height`, layer.height ?? '')
          variables.set(`mix_${id}_${type}_layer_${layer.index + 1}_cropx1`, layer.cropX1 ?? '')
          variables.set(`mix_${id}_${type}_layer_${layer.index + 1}_cropx2`, layer.cropX2 ?? '')
          variables.set(`mix_${id}_${type}_layer_${layer.index + 1}_cropy1`, layer.cropY1 ?? '')
          variables.set(`mix_${id}_${type}_layer_${layer.index + 1}_cropy2`, layer.cropY2 ?? '')
        }
      }

      variables.set(`mix_${id}_${type}_framedelay`, input.frameDelay ?? 0)
    }
  }
  ...
  variables.set('mix_selected', instance.routingData.mix + 1)
